...
public class SubscriptionManager(ICellManager cellManager) : ISubscriptionManager
{
    ...
    public async Task<QueryResult> HandleSubscribe(string[] tokens, WebSocket webSocket)
    {
        ...
        if (tokens[2].ToUpperInvariant() != "USING") return new Error(ErrorPrefixes.QueryParsingError, "Could not find USING statement at correct position. Correct statement is 'SUBSCRIBE <cell> USING <clause>'");
        ...
        return new QueryResult($"Succesfully subscribed to {cellName} instance");
    }
