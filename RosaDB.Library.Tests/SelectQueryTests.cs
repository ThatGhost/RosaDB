...
namespace RosaDB.Library.Tests
{
    ...
    public class SelectQueryTests
    {
        ...
        public void Setup()
        {
            ...
            fakeData1 = RowSerializer.Serialize(Row.Create(["data1", (long)1], tableColumns).Value).Value;
            fakeData2 = RowSerializer.Serialize(Row.Create(["data2", (long)2], tableColumns).Value).Value;
            fakeData3 = RowSerializer.Serialize(Row.Create(["data3", (long)3], tableColumns).Value).Value;
        ...
        {
            // Arrange
            _mockLogManager.Setup(l => l.GetAllLogsForCellTable(cellName, tableName)).Returns(() => ToAsyncEnumerable(new List<Log>()
            {
                fakeLog1, fakeLog2
            }));
        ...
        {
            // Arrange
            _mockLogManager.Setup(l => l.GetAllLogsForCellInstanceTable(cellName, tableName, new object[] { (long)2 })).Returns(() => ToAsyncEnumerable(new List<Log>()
            {
                fakeLog1
            }));
            _mockLogManager.Setup(l => l.GetAllLogsForCellInstanceTable(cellName, tableName, new object[] { (long)1 })).Returns(() => ToAsyncEnumerable(new List<Log>()
            {
                fakeLog2
            }));
        ...
        public async Task SELECT_ProjectOneColumn_ShouldReturnRowsWithOneColumn()
        {
            ...
            string[] tokens = ["SELECT", "data", "FROM", $"{cellName}.{tableName}", ";"];
            ...
            Assert.That(result.Rows[0].Columns[0].Name, Is.EqualTo("data"));
        }
        ...
        public async Task SELECT_ProjectMultipleColumns_ShouldReturnRowsWithMultipleColumns()
        {
            ...
            string[] tokens = ["SELECT", "data", ",", "id", "FROM", $"{cellName}.{tableName}", ";"];
            ...
            Assert.That(result.Rows[0].Columns[0].Name, Is.EqualTo("data"));
            Assert.That(result.Rows[0].Columns[1].Name, Is.EqualTo("id"));
        }
    }
}